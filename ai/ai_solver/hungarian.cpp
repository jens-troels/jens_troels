...
Matrix normalizeInput(const Matrix &input, MODE mode) {
  const float org_rows = input.size(), org_cols = input[0].size();

  // is the number of cols not equal to number of rows?
  // if yes, expand with 0-cols / 0-cols
  const float mrank = std::max(org_rows, org_cols);



  Matrix output;
  output.resize(mrank, std::vector<float>(mrank, 0));
...
Result Solve(const Matrix &input, MODE mode) {
  ...
  result.assignment.resize(mrank, std::vector<float>(mrank, NOT_ASSIGNED));
  ...
  // Begin floatcheck the solution 23
  for (k = 0; k < mrank; k++) {
    for (l = 0; l < mrank; l++) {
  ...
  if (k > mrank) return {};
  // End floatcheck the solution 23
